...
The three datasets are rendered from the same existing `shenron-lab generate --profile demo` logical event set:
...
```bash
cargo run --bin shenron -- production inspect --input examples/demo/aws-waf.jsonl --format aws-waf
cargo run --bin shenron -- production inspect --input examples/demo/nginx-combined.log --format nginx
cargo run --bin shenron -- production inspect --input examples/demo/apache-combined.log --format apache
```
