...
struct Labels {
    ...
    requests_per_source_ip: &'static str,
    status: &'static str,
    other: &'static str,
...
const EN_LABELS: Labels = Labels {
    ...
    requests_per_source_ip: "requests per distinct source IP",
    status: "status",
    other: "other",
...
const JA_LABELS: Labels = Labels {
    ...
    requests_per_source_ip: "異なる送信元 IP あたりのリクエスト数",
    status: "ステータス",
    other: "その他",
...
fn render_concentration(
    ...
    if let Some(focus) = &concentration.focus {
        ...
        html.push_str(&format!(
            "<p>{}: {}</p><p>{}: {:.1}</p><p class=\"note\">{}</p>",
            html_escape(labels.query_shape),
            html_escape(&focus_query_shape_details(focus, language)),
            html_escape(labels.requests_per_source_ip),
            focus.requests_per_source_ip,
            html_escape(labels.query_shape_note),
        ));
