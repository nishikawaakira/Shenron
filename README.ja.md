...
- `production report --input <run-dir> --output <report.html>`：既存の hunt / concentration 成果物から、パス・IP の棒グラフ、分刻み時系列、hunt triage を含む自己完結オフライン HTML を生成します。生のパスと IP を含む private 成果物であり、JavaScript・外部リソース・外部通信は使いません。DoS・攻撃・侵害・悪性確率・攻撃者特定の判定ではありません。詳細は [Private offline HTML report](docs/html-report.md) を参照してください。
