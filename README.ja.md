...
- `report --input <run-dir> --output <report.html> --lang ja`：既存の hunt / concentration 成果物から、パス・IP の棒グラフ、分刻み時系列、hunt triage を含む自己完結オフライン HTML を生成します。グラフには CSS だけで表示するホバーreadoutがあり、時系列の列にカーソルを合わせると UTC の分と件数、棒に合わせると完全なパス／接続ピアと件数を確認できます。幅の広いグラフと表はレポート内でスクロールできます。`--lang ja` で見出しと安全上の注記を日本語にし、整数は3桁区切りで表示します。生のパスと IP を含む private 成果物であり、JavaScript・外部リソース・外部通信は使いません。DoS・攻撃・侵害・悪性確率・攻撃者特定の判定ではありません。詳細は [Private offline HTML report](docs/html-report.md) を参照してください。
