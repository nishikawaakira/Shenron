...
Artifact timestamps remain normalized UTC RFC 3339 values; human-readable hunt, `explain`, and HTML timeline labels explicitly identify UTC and never use the host's implicit timezone.

Hunt can also label deterministic [declared-versus-observed consistency](docs/declared-observed-consistency.md) outcomes. Missing reference data, unsupported telemetry, and missing observed values remain distinct aggregate `unavailable` results rather than mismatches; only bounded mismatch records retain private declaration/observation values, and no outcome determines impersonation, automation, attack, abuse, compromise, or identity.
...
An optional private [analyst disposition store](docs/analyst-dispositions.md) remembers review states for stable template/request-pattern keys. It changes only disclosed classification counts, never retained findings or CVE/Sigma metrics; opinions remain explicitly analyst-authored rather than Shenron conclusions.
