...
`shenron report --input <run-dir> --output <report.html>` turns existing hunt or concentration artifacts into a private, self-contained offline HTML report with inline SVG path/IP bars, minute timelines, and hunt triage. Charts carry visible CSS-only, no-JavaScript hover readouts (a timeline column shows its UTC minute and request count; a bar exposes the full path/peer and count), and wide charts/tables scroll inside the report. Integer counts use three-digit comma grouping; pass `--lang ja` for Japanese labels and safety notices. It contains raw paths and IPs, uses no JavaScript or external resources, and is visualization for human review rather than a DoS, attack, compromise, malice, or attribution determination; see [private HTML reports](docs/html-report.md).
