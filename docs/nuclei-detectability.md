...
**Report kinds.** Every report names its shape in a `report_kind` field. `shenron-lab nuclei coverage` **without** `--telemetry`, and `shenron-lab nuclei update`, write the frozen coverage report (`report_kind: NUCLEI_COVERAGE_REPORT`) — the only shape `production hunt`/`ablation`/`replay`/`count-hypotheses` and `kev coverage` accept as a frozen input. `shenron-lab nuclei coverage --telemetry <profile>` writes a per-profile detectability assessment (`report_kind: NUCLEI_TELEMETRY_COVERAGE`) whose templates carry `level`/`convertible`/`validated` rather than `protocol`/`conversion_status`/`validation_status`; it is an **analysis-only artifact and is not a frozen hunt input**. If a telemetry report (or any other kind) is passed where a frozen input is required, the loader checks `report_kind` before deserializing the body and fails with a message that names the file, the kind found, the kind required, and the command to regenerate it, rather than an opaque serde field error. A report written before `report_kind` existed carries none and is accepted as a frozen input for backward compatibility.
