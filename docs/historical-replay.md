...
Conservative coverage depends entirely on source-finding request IDs, which nginx/Apache combined logs never record. When no loaded finding carries a request ID, coverage is unreachable no matter what the corpus contains, so replay emits an up-front warning to stderr naming this telemetry limitation **before** streaming the (potentially very large) corpus, rather than reporting `unavailable` only after the scan. This is a warning, not an error: the aggregate match volumes remain meaningful and are still produced.

Aggregate `known_findings` counts distinct source finding records, so one finding that references several CVEs contributes one aggregate finding. Per-CVE `known_findings` instead counts every finding that references that specific CVE; this distinction prevents multi-CVE source findings from inflating the aggregate coverage denominator.
